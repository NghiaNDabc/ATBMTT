...
from PyQt6.QtWidgets import QApplication, QTableView, QVBoxLayout, QWidget, QStyledItemDelegate, QCheckBox, QPushButton
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Tạo nút button để hiển thị các dữ liệu được checked
        self.show_checked_button = QPushButton("Hiển thị Checked")
        self.show_checked_button.clicked.connect(self.showCheckedData)
        ...
        self.layout().addWidget(self.show_checked_button)

    def showCheckedData(self):
        checked_data = []
        for row in range(self.model.rowCount()):
            if self.model.data[row][0]:  # Kiểm tra ô checkbox có được checked không
                checked_data.append(self.model.data[row][1:])  # Lấy tên và tuổi của người được checked

        if checked_data:
            print("Các dữ liệu được checked:")
            for data in checked_data:
                print(f"Tên: {data[0]}, Tuổi: {data[1]}")
        else:
            print("Không có dữ liệu nào được checked.")
